...
public:
    PageManager() : mappedData(nullptr), mappedSize(0), fileCapacity(0)
#ifdef _WIN32
    ...
    bool open(const std::string& fname) {
        ...
        // Map view
        mappedData = static_cast<uint8_t*>(MapViewOfFile(hMapping, FILE_MAP_ALL_ACCESS, 0, 0, 0));
        if (!mappedData) {
        ...
        mappedSize = fileCapacity;
        mappedData = static_cast<uint8_t*>(mmap(nullptr, mappedSize, 
    ...
    void* getPage(uint32_t pageId) {
        ...
        if (offset >= mappedSize) {
            if (!grow(pageId + 1)) return nullptr;
        }
...
private:
    bool grow(uint32_t requiredPages) {
...
#else
        // Extend file
        if (ftruncate(fd, newSize) != 0) return false;
        ...
        if (newMap == MAP_FAILED) {
            ...
            munmap(mappedData, mappedSize);
            mappedData = static_cast<uint8_t*>(mmap(nullptr, newSize, 
                PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0));
            if (mappedData == MAP_FAILED) {
                mappedData = nullptr;
                return false;
